...
def get_transaction_by_id(
    ...
    transaction = session.exec(
        select(Transactions).where(
            transaction_id == Transactions.id, Transactions.owner_id == current_user.id
        )
    ).first()
    if not transaction:
        raise HTTPException(
...
@router.put(
    "/{transaction_id}",
    response_model=TransactionsRead,
...
def update_transaction(
    ...
    updated_tx = session.exec(
        select(Transactions).where(
            Transactions.id == transaction_id, Transactions.owner_id == current_user.id
        )
    ).first()
    if not updated_tx:
        raise HTTPException(
